...
class PPChannel(Document):
	...
	def on_trash(self):
		...
		if frappe.db.exists("PP Channel Member", {"channel_id": self.name, "user_id": frappe.session.user, "is_admin": 1}):
			pass
		...
		frappe.db.delete("PP Channel Member", {"channel_id": self.name})

		# delete all messages when channel is deleted
		frappe.db.delete("PP Message", {"channel_id": self.name})

	def after_insert(self):
		# add current user as channel member
		return
		if self.type == "Private" or self.type == "Public":
			frappe.get_doc({"doctype": "PP Channel Member",
	...
	def validate(self):
		...
		if self.type == "Private" or self.type == "Public":
			if self.owner == frappe.session.user and frappe.db.count("PP Channel Member", {"channel_id": self.name}) <= 1:
				pass
			elif frappe.db.exists("PP Channel Member", {"channel_id": self.name, "user_id": frappe.session.user}):
				pass
			elif frappe.session.user == "Administrator":
				pass
			else:
				frappe.throw(
					_("You don't have permission to modify this channel"), 
					frappe.PermissionError
				)
	...
	def add_members(self, members, is_admin=0):
		for member in members:
			doc = frappe.db.get_value("PP Channel Member", filters={
